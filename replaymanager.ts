...
import mimetypes from 'shared/util/mimetypes.json' with { type: "json" };
import zlib from "zlib";
import storage from "./storage.js";// import { encode } from 'acos-json-encoder';
        ...
        if (copy.type == "gameover") {
            try {
                if (roomMetas[room_slug].mode != "rank") {
                    ...
                    delete roomMetas[room_slug];
                    console.warn(
                        "Replays only created for RANK modes: " + room_slug,
                        roomMetas[room_slug].mode
                    );
                    return;
                }
...
async function saveReplay(room_slug) {
    return new Promise(async (rs, rj) => {
        ...
        try {
            ...
            let filename = `${room_slug}.json`;
            let Key = `g/${meta.game_slug}/replays/${filename}`;
